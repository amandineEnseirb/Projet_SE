...
volatile unsigned char fifo_to_send[FIFO_SIZE];
volatile unsigned int head_fifo = 0, back_fifo = 0;
volatile bool is_capture_on = false;
volatile unsigned char accumulateur = 0, counter = 0;
volatile unsigned char buff[] = {0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x02, 0x03, 0x00, 0x01, 0x02, 0x03, 0x00, 0x01, 0x02, 0x03, 0x02, 0x03, 0x02, 0x03};
...
{
	volatile unsigned int tmp = (back_fifo + 1) & 0x3FF;//Remplace le modulo
	if(tmp == head_fifo)
		return false;
